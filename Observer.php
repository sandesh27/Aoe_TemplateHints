...
class Aoe_TemplateHints_Model_Observer {
  ...
  CONST TYPE_CACHED = 'cached';
	...
	protected function getBlockInfo(Mage_Core_Block_Abstract $block, $fullInfo=true) {
		...
		$array_methods = get_class_methods(get_class($block));
	
		
		$rClass = new ReflectionClass(get_class($block));
		$pClassName = $rClass->getParentClass()->getName();
		
		$info['parent Class'] = $pClassName;
		
		$array_parent_methods = get_class_methods($pClassName);
		
		$comma_separated_parent = implode("<br>", $array_parent_methods);
		
		$array_methods_copy = $array_methods;
		
		$array_common_methods = array_intersect($array_methods, $array_parent_methods);
		
		
		if(count($array_common_methods)) {

			foreach ($array_common_methods as $del_val) {
				if(($key = array_search($del_val, $array_methods_copy)) !== false) {
				 	unset($array_methods_copy[$key]);
				}
			}
		}
		$comma_separated = implode("<br>", $array_methods_copy);
		
		$info['class Methods'] = $comma_separated;
		
		$info['parent Class Methods'] = $comma_separated_parent;
